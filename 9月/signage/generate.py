"""
入口サイネージ 30秒動画（グラフィック版・9月仕様）

・縦 1080x1920 → 90°回転で 1920x1080（左傾け設置用）
・9月版: 9〜2月 3,300円（3,630円税込）/ 8月表記なし
...
def draw_qr_panel(
    ...
    if a <= 0:
        return
    pulse = 1.0 + 0.008 * math.sin((t - 21.5) * 3) if t > 21.5 else 1.0
...
def scene_open(draw, t, a, sy):
    y0, y1, y2, y3, y4 = stack_ys([90, 120, 120, 88, 90])
    ...
    draw_band(draw, y4, "9/13(日)まで · 先着30名", 52, a, sy)
...
def scene_offer(overlay: Image.Image, draw, t, a, sy):
    """メイン訴求：6ヶ月間の会費値引き 3,300円（LP準拠）。"""
    if a <= 0:
        return
    local = max(0.0, t - 3.5)
    pulse = 1.0 + 0.006 * math.sin(local * 3.0)

    y0, y1, y2, y3, y4, y5, y6 = stack_ys([88, 150, 100, 80, 520, 72, 56])
    ...
    draw_head(draw, y4 - 72, "3,300", int(280 * pulse), a4, sy, stroke=12)
    draw_head(draw, y4 + 118, "円", 72, a4, sy, stroke=6)
    draw_micro(draw, y5, "3,630円(税込)/月", a5, sy, size=46, stroke=5)
    draw_micro(draw, y6, "※9月会費は入会日からの日割り", a6, sy, size=38, stroke=5)
...
def scene_qr(overlay: Image.Image, draw, t, a, sy):
    ...
    draw_micro(draw, y5, "9/13(日)まで · 先着30名", a, sy, size=52)
